...
entry main  [nri] [nrip1] [nrj] [num_particles] [histo_len]
  ...
  let inner_body (oind: i32) (k: i32) =
      ...
      let jnr               = unsafe jjnr[k]
      let j3                = 3*jnr
      let jx1               = unsafe pos[j3]
      let jy1               = unsafe pos[j3+1]
      let jz1               = unsafe pos[j3+2]
      let dx11              = ix1 - jx1
      let dy11              = iy1 - jy1
      let dz11              = iz1 - jz1
      ...
      let tjA               = ntiA + 2 * (unsafe types[jnr])
      let vnb6              = rinvsix * (unsafe nbfp[tjA])
      let vnb12             = rinvsix * rinvsix * (unsafe nbfp[tjA+1])
      let qq                = iqA * (unsafe charge[jnr])
      let vcoul             = qq*rinv11
      let fs11              = (twelve*vnb12-six*vnb6+vcoul)*rinvsq11
      let tx11              = dx11*fs11
      let ty11              = dy11*fs11
      let tz11              = dz11*fs11
      in  ([tx11, ty11, tz11], jnr, unsafe iinr[oind])
  ...
  let faction' = map (\i-> unsafe faction_iinr'[i] + unsafe faction_jjnr'[i]) (iota histo_len)
  in  faction'
