...
public:
    // vdataR one vector and vval is the other one, and from this two it will construct the histograms
    // no_bins: 2 elems: number of bins for each histograms.
    // vndim = number of dims = 2
    // vdet contains value one in the python code.
    // range: the data from min and maximum (lb1,ub1,lb2,ub2) from vdataR and vval
...
protected:
    ...
    // twqo images pI and pR of equal length
    // N is the size of the vdataR vector
...
public:
    ...
    void locally(int ksize,int* grid_dim, int stride, double* diff, double* val)
    {
        ...
        void locally2d(int ksize,int* grid_dim, int stride, double* diff, double* val)
    {
        vector<int> idx(ksize*ksize,0);
        int end=ksize-1;
        int start=0;
        vector<double> t_val(1,0);
        int offset_stride=1;
        ...
        for(int ii=0;ii<ksize;ii++)
            for(int jj=0;jj<ksize;jj++)
                    idx[nn++]=ii+(jj)*grid_dim[0];
        
        
        for(int i=start;i<grid_dim[0]-end;i+=stride){
            for(int j=start;j<grid_dim[1]-end;j+=stride){
                    offset_stride=i+(j)*grid_dim[0];
                    run(idx.size(),diff,t_val.data(),idx.data(),offset_stride);
                    val[0]+=t_val[0];
    ...
    void locally2d(int ksize,int* grid_dim, int stride, double* diff, double* val, unsigned char* mask)
    {
        vector<int> idx(ksize*ksize,0);
        int end=ksize-1;
        int start=0;
        int center=(int) floor(ksize/2.0)*(grid_dim[0]);
        vector<double> t_val(1,0);
        int offset_stride=1;
        int nn=0;
        for(int ii=0;ii<ksize;ii++)
